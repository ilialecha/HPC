...
main (int argc, char *argv[])
{
  ...
  if (argc < 5) 
    {
      ...
      fprintf (stderr, " %s Seq1 Seq2 SM gapPenalty [N]\n", argv[0]);
  ...
  if (argc >= 6)
    N = atoi (argv[5]);
  if (argc > 6)
    printf ("WARNING: Additional parameters after argv[5]=%s not used.\n",
	    argv[5]);
  ...
  printf ("Max=%d xMax=%d yMax=%d\n", Max, xMax, yMax);
  printf ("Last element in matrix: h[%d][%d]=%d\n", dim1, dim2, h[dim1][dim2]);
  ...
  free(s1); free(s2); free(h); free(hptr); 
